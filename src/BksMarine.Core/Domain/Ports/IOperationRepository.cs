...
public interface IOperationRepository
{
    ...
    Task<List<Operation>> ListAsync(OperationType? type, DateTime? from, DateTime? to, CancellationToken ct = default);
    Task<List<OperationReportRow>> ListReportAsync(OperationType? type, DateTime? from, DateTime? to, Guid? portId, CancellationToken ct = default);
