...
{
    Task<Result<OperationReportFile>> ExecuteAsync(OperationType? type, DateTime? from, DateTime? to, Guid? portId, CancellationToken ct = default);
}
...
public sealed class GenerateOperationReport : IGenerateOperationReport
{
        ...
        OperationType? type, DateTime? from, DateTime? to, Guid? portId, CancellationToken ct = default)
    {
        ...
        var rows = await _operations.ListReportAsync(type, from, to, portId, ct);
        var data = new OperationReportData(from, to, type, portId, rows);
